...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    if(decision.isDenied() && decision.reason.isRateLimit()) {
        logger.warn('Rate Limit excedeed', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path });
    ...
    res.status(500).json({error: "Internal server error", nessage: "Something went wrong with the security middleware"})
  }
}
